...
def get_info_from_database(client, notion_database_id, article_id: int = 0):
    ...
    article_page_id = database_content[article_id]["properties"]["URL"]["url"].split(
        "-"
    )[-1]
    date = database_content[article_id]["properties"]["Date"]["date"]["start"]
    keywords = [
        i["name"]
        for i in database_content[article_id]["properties"]["Étiquettes"][
            "multi_select"
        ]
    ]
    titre_article = database_content[article_id]["properties"]["Nom"]["title"][0][
        "text"
    ]["content"]
    description = database_content[article_id]["properties"]["Description"][
        "rich_text"
    ][0]["text"]["content"]
...
def main(article_id: int = sys.argv[1]):
    ...
    (
        ...
        client=client, notion_database_id=DATABASE_ID, article_id=article_id
    )
